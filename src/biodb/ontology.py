...
# ─── Generic owlready2 helpers (merged from biodb.ontology_owl) ─────────────
# These complement the MONDO-specific load_mondo_ontology above with
# ontology-agnostic primitives that work for any OBO Foundry OWL file:
# Sequence Ontology (SO), HPO, EFO, GO, ChEBI, ... Lazy owlready2 imports
# inside each function keep them optional.
...
_owl_logger = _logging.getLogger(__name__ + ".owl")
...
ReturnAs = _Literal["entity", "label", "id", "id|label"]
Multi = _Literal["join", "first", "all"]
KinType = _Literal["descendants", "ancestors"]
...
def get_labels(ont: Any, unnest: bool = True) -> list[Any]:
    ...
    if unnest:
        return list(_chain.from_iterable(labels))
    return labels
...
def get_ids(ont: Any, unnest: bool = True) -> list[Any]:
    ...
    if unnest:
        return list(_chain.from_iterable(ids))
    return ids
...
def _get_kin(
    ...
    if entity is None:
        if verbose:
            _owl_logger.info("No entity found for %r", label_or_id)
    ...
    if verbose:
        _owl_logger.info("Found %d %s of %r", len(kin), kin_type, label_or_id)
    return list(map(_partial(_return_as, return_as=return_as), kin))
