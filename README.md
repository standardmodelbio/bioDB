...
| **[Ontology Lookup Service (OLS)](https://www.ebi.ac.uk/ols4/)** — EBI-hosted gateway to ~280 OBO Foundry ontologies (Mondo, HPO, EFO, SO, GO, ChEBI, SNOMED, …) | [`biodb.ols`](src/biodb/ols.py), [`biodb.ontology`](src/biodb/ontology.py) | ✅ OLS4 REST — no local file needed. Ranked term lookup (`find_terms`, `find_term`), hierarchy traversal (`get_term`, `get_descendants`, `get_ancestors`, `get_children`, `get_parents`), Solr full-text search (`search`), CURIE↔IRI conversion (`curie_to_iri`, `ontology_id_from_curie`). Also generic `owlready2` entity walk for callers with a local OWL file (`ontology.get_ontology`, …). See [`docs/ols.md`](docs/ols.md). | ✅ Whole-ontology dumps with version-aware on-disk parquet cache (`list_terms`); OBO / OWL download + parse, N-hop expansion, hierarchical keyword sets, attention-weight analysis, gene-phenotype matrix, ontological similarity |
...
| **[SNOMED CT](https://www.snomed.org/)** (OHDSI-flavoured) — clinical terminology: diagnoses, procedures, findings, body sites | [`biodb.snomed`](src/biodb/snomed.py) | ✅ Per-concept lookups via OLS4 (`query_concept`, `search_concepts`, `get_descendants`, `get_ancestors`, `get_children`, `get_parents`) | ✅ Local CONCEPT.csv parser (`load_concept_csv`, `load_concept_csv_from_zip`). SNOMED CT licensing prohibits us from redistributing bytes — obtain a vocabulary bundle from [OHDSI Athena](https://athena.ohdsi.org) after accepting the SNOMED CT license, then point bioDB at the local file. |
